...
2. If the user has not approved a visual direction yet, use `slides-grab list-styles` to shortlist bundled styles, optionally `slides-grab preview-styles` for a local HTML gallery, and agree on a direction with the user. If none of the 35 bundled styles fit, design a fully custom visual direction.
